...
const PAGES: PageCard[] = [
  ...
  {
    title: "Messages",
    description: "Send and receive messages from colleagues. Task requests and replies.",
    href: "/messages",
    color: "text-indigo-500",
    gradient: "from-indigo-500/20 to-indigo-600/5",
    pageKey: "messages",
  ...
  {
    title: "Audit Log",
    description: "Detailed activity log for compliance. Who did what, when. Export for audits.",
...
export function DashboardHome({ profile }: { profile: Profile }) {
  ...
  const visiblePages = PAGES.filter((p) => {
    ...
    if (p.pageKey === "request") return ["admin", "operations", "manager"].includes(profile?.role ?? "");
