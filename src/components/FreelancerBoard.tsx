...
export function FreelancerBoard({
  ...
  const bulkMoveToGroup = useCallback(async (groupKey: string) => {
    ...
    } else if (groupKey === "admin_approvals") {
  ...
  const exportToPdf = useCallback(async (data: DisplayRow[]) => {
    ...
    autoTable(doc, {
      startY: 25, styles: { font: "Roboto", fontSize: 7, cellPadding: 2 }, headStyles: { fillColor: [59, 130, 246] },
...
function StatusCell({ r, canApprove, isSubmitter, currentRole, isOperationsRoomMember, actionLoadingId, onManagerApprove, onAdminApprove, onResubmit, onMarkPaid, openRejectModal }: {
  ...
  if (r.status === "rejected") return <div className="flex items-center justify-center gap-1" onClick={e => e.stopPropagation()}><span className="inline-flex h-7 w-7 items-center justify-center rounded-full text-white shadow-sm" style={statusStyle} title={r.rejectionReason || "Rejected"}>✗</span>{(isSubmitter || currentRole === "admin") && currentRole !== "viewer" && btn(() => onResubmit(r.id), "Resubmit", "↻", "bg-emerald-500 hover:bg-emerald-600 text-sm")}</div>;
...
function CompareTable({ rows, ids }: { rows: DisplayRow[]; ids: string[] }) {
  ...
  const fields: { label: string; key: keyof DisplayRow }[] = [
    ...
    { label: "Beneficiary", key: "beneficiary" }, { label: "Sort Code", key: "sortCode" },
    { label: "Account Number", key: "accountNumber" }, { label: "INV Number", key: "invNumber" },
