/**
 * Bulk send invites from producer guests flow.
 * Same logic as invite-send but for multiple guests.
 */
...
export async function POST(request: NextRequest) {
  try {
    ...
    const body = (await request.json()) as {
      guests: { producer_guest_id?: string; guest_name: string; email: string; title: string }[];
      program_name: string;
      topic: string;
      record_date: string;
      record_time: string;
    ...
    const guests = (body.guests ?? []).filter(
      (g) => g.guest_name?.trim() && g.email?.trim()?.includes("@") && g.title?.trim()
    );
    if (guests.length === 0) {
      return NextResponse.json({ error: "At least one guest with name, email and title is required" }, { status: 400 });
    }
    ...
    for (let i = 0; i < guests.length; i++) {
      const g = guests[i];
      const guestName = g.guest_name.trim();
      ...
      if (i < guests.length - 1) await sleep(BATCH_DELAY_MS);
    }

    return NextResponse.json({ success: true, message: `Invitation sent to ${sent} of ${guests.length} guest(s).` });
  } catch (e) {
