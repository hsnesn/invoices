...
async function parseNaturalLanguage(text: string, availableRoles: string[]): Promise<ParsedRequest | null> {
  const apiKey = process.env.OPENAI_API_KEY;
  if (!apiKey) return null;

  const client = new OpenAI({ apiKey });
  const rolesList = availableRoles.length > 0 ? availableRoles.join(", ") : "Output, Director, Camera, etc.";
...
- role: one of [${rolesList}] or closest match. "output" or "outputs" usually means "Output".
- count_per_day: integer 1-20
- days_of_week: array of 0-6 (0=Sun, 1=Mon, 2=Tue, 3=Wed, 4=Thu, 5=Fri, 6=Sat). "weekdays"/"week days"/"Mon-Fri" = [1,2,3,4,5], "weekends" = [0,6], "every day" = [0,1,2,3,4,5,6]`,
      },
      {
        role: "user",
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (text) {
      ...
      const aiParsed = await parseNaturalLanguage(text, availableRoles);
      if (!aiParsed) {
