...
function invitationEmailHtml(inviterName: string, recipientName: string, role: string, magicLink: string) {
  const roleBadgeColor = { admin: "#dc2626", manager: "#2563eb", finance: "#059669", operations: "#ea580c", submitter: "#6b7280", viewer: "#64748b" }[role] ?? "#6b7280";
  return `
<!DOCTYPE html>
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { email, full_name, role, department_id, program_ids } = body as {
      email: string; full_name?: string; role: "submitter" | "manager" | "admin" | "finance" | "viewer" | "operations";
      department_id?: string | null; program_ids?: string[] | null;
    };

    if (!email || !role) return NextResponse.json({ error: "email and role are required" }, { status: 400 });
    if (!["submitter", "manager", "admin", "finance", "viewer", "operations"].includes(role)) return NextResponse.json({ error: "Invalid role" }, { status: 400 });
