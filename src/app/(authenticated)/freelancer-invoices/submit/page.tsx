...
export default function FreelancerSubmitPage() {
  ...
  const [serviceDescriptions, setServiceDescriptions] = useState<SetupItem[]>([]);
  const [costReasons, setCostReasons] = useState<SetupItem[]>([]);
  const [bookedByOptions, setBookedByOptions] = useState<SetupItem[]>([]);
  ...
  useEffect(() => {
    ...
    fetch("/api/freelancer-setup?category=service_description").then((r) => r.json()).then((d) => { if (Array.isArray(d)) setServiceDescriptions(d); }).catch(() => {});
    fetch("/api/freelancer-setup?category=additional_cost_reason").then((r) => r.json()).then((d) => { if (Array.isArray(d)) setCostReasons(d); }).catch(() => {});
    fetch("/api/freelancer-setup?category=booked_by").then((r) => r.json()).then((d) => { if (Array.isArray(d)) setBookedByOptions(d); }).catch(() => {});
    ...
    <div className="mx-auto max-w-2xl py-8 px-4">
      ...
      <form onSubmit={handleSubmit} className="space-y-6 rounded-2xl border border-gray-200 bg-white p-6 shadow-lg dark:border-gray-700 dark:bg-gray-800">
        ...
        <div>
          ...
          <p className={hintCls}>Provision of freelance editorial production services, delivered on a project basis, charged per service delivery day</p>
          <div className="relative mt-2 rounded-lg border-2 border-dashed border-gray-300 bg-gray-50 px-6 py-10 text-center dark:border-gray-600 dark:bg-gray-700/30 hover:border-teal-400 transition-colors">
            <input type="file" accept=".pdf,.docx,.doc,.xlsx,.xls" onChange={(e) => setFile(e.target.files?.[0] ?? null)} className="absolute inset-0 cursor-pointer opacity-0" />
            {file ? (
              <div className="flex items-center justify-center gap-2">
                <svg className="h-5 w-5 text-teal-500" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                <span className="text-sm font-medium text-gray-700 dark:text-gray-300">{file.name}</span>
                <button type="button" onClick={(e) => { e.preventDefault(); setFile(null); }} className="ml-2 text-xs text-red-500 hover:text-red-700">Remove</button>
              </div>
            ) : (
              <div>
                ...
                <p className="mt-2 text-sm text-teal-600 font-medium">Choose a file to upload<span className="text-gray-400 font-normal"> or drag and drop here</span></p>
                <p className="text-xs text-gray-400 mt-1">PDF, DOCX, DOC, XLSX, XLS</p>
              </div>
            )}
          </div>
        </div>
