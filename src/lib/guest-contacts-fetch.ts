  ...
  if (search && search.length >= 2) {
    ...
    if (error) {
      ...
      const { data: fallback } = await supabase
        .from("guest_contacts")
        .select("id, guest_name, phone, email, title, title_category, topic, topic_category, ai_contact_info, ai_searched_at, ai_assessment, ai_assessed_at, updated_at, is_favorite, tags, affiliated_orgs, prohibited_topics, conflict_of_interest_notes")
        .order("guest_name");
      return fallback ?? [];
    }
    return data ?? [];
